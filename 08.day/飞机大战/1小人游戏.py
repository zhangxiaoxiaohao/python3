...
class goldrect(pygame.sprite.Sprite):#绘出金币
    def __init__(self,gold_position,speed):
        ...
        self.image=pygame.image.load('images\\jinbi.jpg')
        self.rect=self.image.get_rect()
...
def drawback(): #绘出背景图片
    my_back=pygame.image.load('images\\background.png') 
...
while True:
    ...
    if pressed_keys[pygame.K_LEFT]:#按下左键
        ...
        iright=iright+1
        filename='images\\'+str(iright)+'.png'
        if x<50 :
            x=50
        else:
            x=x-10
